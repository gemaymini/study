...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None):
        # downsample对应虚线残差结构conv3,4,5的第一层卷积都要采用下采样的方式,把downsample设为Ture
        super(BasicBlock, self).__init__()
        self.conv1 = nn.Conv2d(in_channels=in_channel, out_channels=out_channel,
                               kernel_size=3, stride=stride, padding=1, bias=False)  # stride=1对应实线残差结构,stride=2对应虚线结构
...
class ResNet(nn.Module):
    ...
    # block_num为残差结构中conv2_x~conv5_x中残差块个数，是一个列表具体查看img.png如34层残差结构列表为[3,4,6,3]
    # num_classes是训练集的类数
    # include_top是为了方便以后添加别的框架
    def __init__(self, block, blocks_num, num_classes=1000, include_top=True):
        super(ResNet, self).__init__()
        self.include_top = include_top
        self.in_channel = 64  # maxpooling层后的输入参数,都是64
